...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		if(MsgID == NETMSGTYPE_CL_SAY)
		{
			...
			{
				char aCommand[4][256] = {0};
				int Command = 0;
				int Char = 0;
				// Analizing of the Command
				for(unsigned int i = 1;i<str_length(pMsg->m_pMessage);i++)
				{
				...
				{
					SendChatTarget(ClientID, "Unknown command. Try \"/cmdlist\", \"/animals\" or \"/help\"");
				}
			} else
...
//animal
void CGameContext::ConResetWalls(IConsole::IResult *pResult, void *pUserData)
{
    CGameContext *pSelf = (CGameContext *)pUserData;
    int ClientID = clamp(pResult->GetInteger(0), 0, (int)MAX_CLIENTS-1);
	for(int i = 0; i < MAX_CLIENTS; i++) {
		...
		//pSelf->m_apPlayers[i]->WallReset = 1;

		pSelf->m_pController->wallreset = 1;
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("resetWalls", "r", CFGFLAG_SERVER, ConResetWalls, this, "Reset the walls");
}
